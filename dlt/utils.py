...
def round_float(value, ref_value, tol_magnitude=1.0e-6):
    """
...
def extract_independent_columns(matrix, DEBUG=False, **kwargs):
    ...
    for col_idx in range(1, matrix.shape[1]):
        ...
        if np.linalg.matrix_rank(candidate_matrix, **kwargs) > np.linalg.matrix_rank(
            current_matrix, **kwargs
        ):
...
def build_fk_matrix(fk_dict):
    ...
    FK = np.vstack(
        [
            fk.reshape((fk.shape[0], fk.shape[1] * fk.shape[2]))
            for fk in fk_dict.values()
        ]
    )
    ...
    try:
        ...
        result_flatten = result.reshape(
            (result.shape[0] * result.shape[1], result.shape[2])
        )
        assert np.allclose(result_flatten, mat_prod)
        assert np.allclose(
            result,
            mat_prod.reshape((result.shape[0], result.shape[1], result.shape[2])),
        )
...
def regularize_matrix(M, tol: float = None):
    ...
    if is_symmetric and is_squared:
        ...
        if tol is None:
            tol = np.amax(eigvals, initial=0.0) * np.finfo(M.dtype).eps
        ...
        if tol is None:
            tol = np.amax(S, initial=0.0) * np.finfo(M.dtype).eps
