...
kernel learning, \ie, by applying it to an initial condition that represents our
prior. In the following, we choose the initial condition to be an ensemble of
...
solution (TS), obtained after training a neural network using GD and 
...
$U$, $\Delta[U f_0]$ is different from zero for small training times, and thus
the correlations between $U(t)$ and $f_0$ are non-negligible. However, as
training proceeds, $\Delta[U f_0]$ becomes compatible with zero within the error
bars, suggesting that the correlations are progressively suppressed. The case of
the $V$ operator is even more striking, as $\Delta[V Y]$ is clearly
...
fields. This is an important step in our study of the error estimates, since it 
guarantees that the error bars computed from the ensemble of trained PDFs is not 
biased by the choice of prior
that is made by selecting a given architecture, activation function, and
probability distribution for the biases and weights at initialisation.
...
the definition of the matrix $M$ given in Eq.~\eqref{eq:MandBDef}, which encodes
the physical information enclosed in the FK tables. The bias thus assumes a
...
the contribution of the $U$ contribution is significant only at early epochs. In
...
of the timescale $1/h_i$, shown as vertical dashed lines in grey, the variance
shows a change in slope. This remains true across all closure-test data,
although slightly mitigated for L2 data.

We remark that the interplay between bias and variance, as well as the presence
of extrema in both curves, may have practical implications in the choice of the
optimal stopping time. For this reason, in Fig.~\ref{fig:BiasVarianceComparison}
we show the total contributions of bias and variance together as functions of
the training time for different frozen NTKs and for L0, L1 and L2 data.
...
\begin{figure}[!h]
  ...
  first column, the bias is off-scale.
  \label{fig:BiasVarianceComparison}}
\end{figure}

% \begin{figure}[!h]
%   \centering
%   \includegraphics[width=0.90\textwidth]{section_4/bias_variance_L0_func.pdf}
%   \includegraphics[width=0.90\textwidth]{section_4/bias_variance_L1_func.pdf}
%   \includegraphics[width=0.90\textwidth]{section_4/bias_variance_L2_func.pdf}
%   \caption{Same as Fig.~\ref{fig:BiasVarianceComparison}, but measuring bias and variance in the
%   functional space of the PDF.}
%   \label{fig:BiasVarianceComparisonFunc}
% \end{figure}
