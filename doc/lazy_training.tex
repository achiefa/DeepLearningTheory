...
determined by the initial condition, \ie\ by the prior distribution in
functional space.}
\begin{align}
...
as the flow (or training) Hamiltonian, training can only take place in the space
...
quite rapidly, as a consequence of the fact the time scales in the analytical
...
In conclusion, we note that Eq.~\eqref{eq:MeanValAtTNoCorr} resembles the
structure of a linear method, like \eg\ Backus-Gilbert or Gaussian Processes. We
...
fields. This is a crucial point, since it guarantees that the error bars
computed from the ensemble of trained PDFs is not biased by the choice of prior
...
inversion is not exact even with level-0 data 
\begin{equation}
  \fin_{M^{\perp}} = M^{+} (FK)^T C_Y^{-1} Y_{L0} \neq \fin \,,
  \label{eq:NaiveInversion}
\end{equation}
where $M^{+}$ is the pseudo-inverse of $M$. As in the case of the NTK, there is
a residual component of the input vector in the eigenspace of $M$,
...
Sec.~\ref{sec:Lazy}. Given that this component does not receive any restriction
from the data, the parametrisation, together with the fitting methodology, has
...
\begin{equation}
  \mathcal{V} = \mathcal{V}_{U} + \mathcal{V}_{V} + \mathcal{V}_{UV} \,,
\end{equation}
...
will depend on the data used in the training.} We can then rewrite the combined
...
subspace, applying the pseudoinverse and finally recomputing the full field as
detailed above.
