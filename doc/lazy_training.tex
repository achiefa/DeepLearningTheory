...
\begin{figure}[h!]
  ...
  Eq.~\eqref{eq:FlowEquationNoIndices}, at different training times and as a
...
first five eigenvalues of $H^{\perp}$ for the three different closure datasets.
In Fig.~\ref{fig:HEigVecs} we show the first five eigenvectors of $H$, denoted
as $q^{(i)}$, at different training times as functions of the $x$-grid. It
should not come as too much of a surprise that the eigenvalues and eigenvectors
of $H^{\perp}$ have a similar behaviour to those of the NTK (see
Figs.~\ref{fig:NTKEigvalsTime} and \ref{fig:NTKEigVecs}), from which they are constructed.
However, we see that $h^{(i)}$ are larger by around three orders of magnitude
than the NTK eigenvalues.
...
\begin{align}
    \label{eq:HomoSoln}
    \hat{f}^{\perp}_{t,k} = \sum_{i=1}^{d^\perp} f^{\perp (i)}_{0} e^{-h^{(i)}t} w^{(i)}_k\, ,
\end{align}
where
...
\begin{align}
    \label{eq:InitialCi}
    f^{\perp (i)}_{0} = \sum_{k=1}^{d_\perp} w^{(i)}_k f^\perp_{0,k}\, ,
...
training, for different choices of the frozen NTK. The NN is trained using the
numerical GD until $t_{\rm ref}$, at which point the NTK is frozen. The
evolution time $t$ used in the analytical solution is the difference between the
total training time and $t_{\rm ref}$; the intial condition for the analytical
solution is the trained solution at $t_{\rm ref}$. Central value and uncertainty
bands are obtained by computing the analytical solution for each replica of the
initial condition and frozen NTK.\footnote{Unless stated otherwise, in this
section central values and uncertainties are always computed as ensemble
averages across replicas.} The figures agree with the expectation; the closer
$t_{\rm ref}$ is to the onset of the lazy regime, the better the agreement
between the analytical solution and the trained function.
% Onset Lazy L2 ===================================
\begin{figure}[ht!]
  \centering
...
\begin{figure}[ht]
    ...
    condition for the analytical solution is always $f_{T_{\rm ref}}$. 
    As in Fig.~\ref{fig:OnsetLazyL2}, L2 data is used.}
...
solution can be used to perform kernel learning. We address this question in 
Sect.~\ref{sec:NTKKernelLearning}, after deriving a few additional analytical results 
...
also depends on the data used in the training.} We can then rewrite the combined
...
\begin{figure}[h!]
  ...
  = 10000$. L2 data is used in the plot. Note that the scale on the vertical 
  axis is three orders of magnitude smaller than in Fig.~\ref{fig:InfiniteTimeVterm}.}
...
solution (TS), obtained by training another ensemble of networks drawn
...
\begin{align}
    ...
    \Delta[V(t)Y] &= \mathbb{E}\left[V(t) Y\right] 
...
Fig.~\ref{fig:xT3_exp_val}, deserves further investigations, which we leave for
...
statistical error on the data. We show the diagonal entries of the two
contributions $C_t^{(00)}$ (blue band) and $C_t^{(YY)}$ (orange band) to the
error budget in Fig.~\ref{fig:ErrorBudgetL2}, for different frozen NTKs (rows)
and different training epochs (columns), using L2 data as before. We do not show
the mixed term $C_t^{(0Y)}$ since it is negligible with respect to the other two
contributions -- the two sources of uncertainty are largely uncorrelated. In
general, we observe that towards the end of the training process the
contribution from the data $C_t^{(YY)}$ becomes dominant with respect to the
contribution coming from the initial condition $C_t^{(00)}$. We also see that
the suppression of the initial condition is more severe and happens earlier when
the frozen NTK is taken at later stages of training.
...
\begin{figure}[ht!]
  \centering
  \includegraphics[width=0.80\textwidth]{section_4/error_buddget_L2_logx_logx.pdf}
...
the data, the parametrization, together with the fitting methodology, has
...
\begin{figure}[!h]
  ...
  \caption{Bias decomposition as in Eq.~\eqref{eq:BiasDecomposition} as a function
...
\begin{figure}[!h]
  ...
  \caption{Variance decomposition as in Eq.~\eqref{eq:VarianceDecomposition} as a function
  of the training time for L0 (top), L1 (middle) and L2 (bottom) data. The curves
  are computed using the analytical result in Eq.~\eqref{eq:AnalyticSol}. Each
