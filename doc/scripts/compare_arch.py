...
PLOT_ARGS = [
    ...
    {"color": "orange", "marker": "s", "label": r"$\textrm{Arch. } [100, 100]$"},
]
...
def produce_eigvals_comparison(args):
    ...
    for i in range(args.nvals):
        ...
        fig.tight_layout()
        fig.savefig(
            PLOT_DIR / f"eigval_{i+1}_arch_{args.data}.pdf", bbox_inches="tight"
        )
        res.append((fig, ax))
...
def produce_delta_ntk_comparison(args):
    ...
    fig.tight_layout()
    fig.savefig(PLOT_DIR / f"delta_ntk_arch_{args.data}.pdf", bbox_inches="tight")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="L0",
        help="Data generation (L0, L1, L2, or real)",
    )
