...
theoretical predictions, while $b$ is a vector that depends only on the data.
...
We discuss the cut-off tolerance in Appendix~\ref{sec:cutoff}.}. This means that
...
(see, \eg, Ref.~\cite{fort2020dlvk} were two similar regimes where also
...
$z^{(k)}$ and the input $f^{\mathrm{in}}$ for two subsects of eigenvectors, in the right and left panel
