...
Fig.~\ref{fig:NTKAlignFin}, we show the cosine of the angle between the eigenvectors
$z^{(k)}$ and the input $f^{\mathrm{in}}$ for two subsets of eigenvectors, 
in the right and left panel
respectively. In these two panels, different patterns can be observed. Firstly,
...
directions that were subdominant in the first stage of training, in agreement 
with the observations coming from Figs.~\ref{fig:EigvalsComparison}-\ref{fig:NtkMAlign}.
By the onset of the lazy training regime, the input function lies entirely in
the subspace spanned by the first four eigenvectors of the NTK. This confirms
that the NTK has aligned its features with the physically relevant directions of
the problem.
...
\begin{figure}[ht!]
  ...
  $\fin$ used to generate the L2 data, measured in terms of $\cos
  \theta^{(k)} = (z^{(k)}, \fin)/ \Vert \fin \Vert$. In the
  left panel, the first four eigenvectors of the NTK are shown, while the right
  panel shows the remaining eigenvectors up to $k=7$. Note that $f^{\mathrm{in}}$
  lies entirely in the subspace spanned by the first four eigenvectors of the NTK 
  by the onset of the lazy training regime. We see that the NTK has aligned its
  features with the physically relevant directions of the problem.}
...
show the eigenvectors of the product of matrices $\Theta_t M$, labelled with
$q^{(i)}$, at different training times and as functions of the $x$-grid. The
product of these two matrices will become relevant in
Sec.~\ref{sec:LazyTraining}, where the solution of the lazy training equations
is expressed as a linear combination of these eigenvectors. 
Together with the eigenvectors, we also show the
...
becomes more structured in order to reproduce the output function. Again, this
...
\begin{figure}[ht!]
  ...
  Eq.~\eqref{eq:FlowEquationNoIndices}, at different training times and as
