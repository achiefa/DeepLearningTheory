...
% Given these expressions, we can solve for $f_t$ using the pseudoinverse of $\FKtab$. However, before we look for the 
% solution for $f_t$ we should check that Eq.~\ref{eq:SolutionForFKf} describes what we obtain by running the 
% usual gradient descent. Note that the matrix $H$ is symmetric and positive definite, so that
% \begin{align}
%     \label{eq:ExpHt}
%     e^{H t} = \sum_i e^{- H_i t} |v_i\rangle \langle v_i|\, .
% \end{align}
