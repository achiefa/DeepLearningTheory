...
the inverse problem of PDF determination for data depending linear in the PDFs,
setting the notation used throughout this work and introducing the statistical
vocabulary used in the rest of the work. We then provide an account for some
...
in the large-width limit~\cite{lee2019wide,jacot2018neural}, are compared to the
specific architecture used in the NNPDF methodology. A exhaustive and detailed
...
solutions with a finite set of data is inherently ill-posed. The solution
...
these challenges, yielding increasingly accurate and precise PDFs. Yet, despite
...
posterior distribution $\tilde{p}(f)$. In this study, and specifically in the
context of the NNPDF methodology, probability distributions are represented by
...
initialisation is discussed below in Sec.~\ref{sec:Init}.

In order to account for the experimental uncertainties and propagate them in the
final PDFs, the NNPDF has proposed the Monte Carlo replica method. For each
replica, a new set of data $Y^{(k)}$ is generated from an $\ndat$ dimensional
...
provides crucial insight into how prior knowledge embedded in the initialization
...
layers is increased. We call this limit the {\em large-network} limit.
...
$\ngrid=50$ points is used to compute observables in the NNPDF formalism and in
