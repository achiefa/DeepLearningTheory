...
Eq.~\eqref{eq:RecursionForK}, accurately reproduces the output distribution
of the NNPDF networks at initialisation, provided that a sufficiently large
ensemble of replicas is used to sample the distribution. Finally, the mean and
the variance of the output at initialisation for all values of $x$ are shown in
Fig.~\ref{fig:prior} for an ensemble with $\nreps=100$. The central value is
computed as discussed above in Eq.~\eqref{eq:ReplicaEnsemble},
\begin{align}
