...
\begin{figure}[!t]
    ...
    We show the case of linear input $f(x)$ (blue) and logarithmic-scaled input
    $f(x, \log x)$ (orange). The solid lines represent the mean value computed
    over an ensemble of 100 replicas, while the shaded bands represent the
    one-sigma uncertainty computed as the variance over the same ensemble. In
    the figure, we show $xT_3$ as used in the following sections.}
    \label{fig:prior}
\end{figure}
