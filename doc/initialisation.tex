...
neuron}\ index.}, and $x_{\alpha}$ is the input to the NN, \ie\ a point in the 
...
\end{align}
Note that the integration variables in Eq.~\ref{eq:FunctIntDef} do not have 
a neuron index and the integrals are $\ngrid$ dimensional integrals. 
Eq.~\eqref{eq:RecursionForK} is iterated for the NNPDF architecture,
yielding $K^{(\ell)}$ for arbitrary $\ell$, \ie\ the covariance at initialisation 
for various depths. These are compared
...
\begin{figure}[t]
    ...
    ensemble of replicas, initialised using the Glorot normal distribution. The covariance in the right
    panel is obtained by solving Eq.~\eqref{eq:RecursionForK} numerically. In order to reduce
    the bootstrap errors in the empirical covariance, an ensemble of 1000 replicas has been used for this
    figure.
...
\begin{figure}[t]
    ...
    third (bottom) layers of the NNPDF architecture. Again, an ensemble of 1000 replicas
    has been used to reduce the bootstrap errors in the empirical covariance.}
...
\begin{figure}[t]
    ...
    kernel obtained by iterating Eq.~\ref{eq:RecursionForK} for the three layers
...
point $x_\alpha$ is given by the diagonal matrix elements of $K^{(\ell)}$. 
The mean and the variance of the output at
initialisation are shown in Fig.~\ref{fig:prior} for an ensemble with
