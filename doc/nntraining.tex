...
In the limit of inifinite training time, the evolution operators $U(t)$ and $V(t)$ simplify to
\begin{align}
    \hat{U}^\perp(t\to\infty)_{\alpha\alpha'} 
        &= 0\, \\
    \check{U}^\perp(t\to\infty)_{\alpha\alpha'}
        &= \sum_{i} Z^{(i)}_{\alpha} \frac{1}{h^{(i)}} \tilde{Z}^{(i)}_{\alpha'}\, , \\
    U^\parallel(t\to\infty)_{\alpha\alpha'}
        &= \sum_{k\in\parallel} z^{(k)}_\alpha z^{(k)}_{\alpha'}\, , \\
    V(t\to\infty)_{\alpha I}
        &= \sum_{i} Z^{(i)}_{\alpha} \frac{1}{h^{(i)}} \tilde{T}^{(i)}_{I}\, .
\end{align}
