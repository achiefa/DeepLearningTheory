...
variation of the NTK vs training time $t$
...
compute the mean value and the variance, for a several values of $t$, similar to the $\Delta\Theta$ plot,
\[
    \bar{U}(t)_{\alpha\alpha'} = \mathbb{E}[U(t)_{\alpha\alpha'}]\, , \quad
        \mathbb{E}\left[\left(U(t)_{\alpha\alpha'} - \bar{U}(t)_{\alpha\alpha'}\right)^2\right]\, .
\]
