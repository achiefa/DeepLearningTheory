...
\end{align}
Using the operator $\mathcal{M}$, introduced in Eq.~\eqref{eq:MOperatorDef},
the evolution operators at infinite training time are
\begin{align}
...
\end{align}
\ldd{To be rewritten using the expression we found in the section above. Should yield a very nice interpretation.}
...
\begin{align}
    ...
    C^{(YY)}
        ...
        - \mathbb{E}\left[V(t) Y\right] \mathbb{E}\left[Y^T V(t)^T\right]\, .
\end{align}
We report the values of $C^{(00)}$, $C^{(0Y)}$ and $C^{(YY)}$ for different values in 
...
the statistical uncertainty of the trained fields. 
